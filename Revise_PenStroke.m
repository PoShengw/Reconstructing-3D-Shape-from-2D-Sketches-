...
for i=1:size(global_theta,2)              %calculte the global_angle
    global_angle = [global_angle; mvnrnd([global_theta(i)],30,1500)];  % (mean/S/cases)    (平均值/標準差/抽取樣本數)  
end
...
for i=1:size(theta,1)
    for j=1:3
     lc_ang=theta(i,:);
     local_angle= [ local_angle;mvnrnd(lc_ang(j),30,size(global_theta,2)*1500/3)];
...
for i=1:size(Edg,1)           % calculate the original depth of each point (select V1 or V2 or V3)
       if i~=origin_point && i~=main_axis(1) && i~=main_axis(2) && i~=main_axis(3)
        ...
        end_point=i;
        [pathway]=minspantree(first_point,end_point);
